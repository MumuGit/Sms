package com.mu.example.sms.sms;
...
public class SmsObserver extends ContentObserver {
    ...
    public void onChange(boolean selfChange, Uri uri) {
        ...
        Uri inboxUri = Uri.parse("content://sms/queued");//收件箱
//        Uri inboxUri =uri;//收件箱
        try {
            Cursor c = mContext.getContentResolver().query(inboxUri, null, null,
                    null, "date desc");
//            Cursor c = mContext.getContentResolver().query(inboxUri, null, "_id=?",
//                    new String[]{"7128"}, "date desc");
            if (c != null) {
                if (c.moveToFirst()) {
//                    while (! c.isAfterLast()){
//
